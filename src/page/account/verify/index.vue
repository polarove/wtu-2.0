<template>
    <div class='page-account-verify'>
        ...
        <div class=" w-80% display-none" :class="{ visible: verificationStatus.succeed }">
            <div class="animate__animated animate__pulse">
                <div class="i-ep:check text-size-[3rem] "></div>
                <div>{{ verificationStatus.message }}向大家介绍一下自己吧！</div>
            </div>
            <el-form class="mt-20px" ref="NameFormRef" :model="NameForm" :rules="NameFormRules">
                <el-form-item prop="name" label="我是：">
                    <el-input v-model="NameForm.name" class="inline-block" placeholder="请填写游戏内昵称，用于组队"
                        @keyup.enter="saveMyName(NameFormRef)">
                        <template #append>
                            <el-button :loading="loading" @click="saveMyName(NameFormRef)">保存</el-button>
                        </template>
                    </el-input>
                </el-form-item>
            </el-form>
